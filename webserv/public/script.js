...
function generateRandomChat() {
  const randChat = chat[Math.floor(Math.random() * chat.length)];
  const container = document.getElementById("chatContainer");

  const p = document.createElement("p");
  p.textContent = randChat;
  ...
  const randomInterval = Math.floor(Math.random() * 400);
  setTimeout(generateRandomChat, randomInterval);
}
